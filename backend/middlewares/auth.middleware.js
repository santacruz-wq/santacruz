...
export const verificarToken = async (req, res, next) => {
    try {
        ...
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return res.status(401).json({ message: "Token requerido, debe loguearse" });
        }
...
export const soloMesero = (req, res, next) => {
    if (req.usuario?.rol !== "mesero") {
        return res.status(403).json({ message: "Acceso denegado. Solo meseros." });
    }
    next();
};
...
export const soloCocina = (req, res, next) => {
    if (req.usuario?.rol !== "cocina") {
        return res.status(403).json({ message: "Acceso denegado. Solo personal de cocina." });
    }
    next();
};